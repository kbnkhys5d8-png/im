...
func TestChannelHandlerProcessingReservationIsExclusive(t *testing.T) {
	h := &channelHandler{}
...
func TestChannelPollerRunsTaskInlineWhenWorkerPoolIsFull(t *testing.T) {
	oldOptions := options.G
	if options.G == nil {
		options.G = options.New()
	}
	oldSize := options.G.Poller.ChannelGoroutine
	options.G.Poller.ChannelGoroutine = 1
	t.Cleanup(func() {
		options.G.Poller.ChannelGoroutine = oldSize
...
func TestChannelHandlerTimeoutRequiresIdleQueue(t *testing.T) {
	oldOptions := options.G
	options.G = options.New()
	options.G.Poller.ChannelTimeout = time.Nanosecond
	t.Cleanup(func() { options.G = oldOptions })

	h := &channelHandler{}
	h.pending.eventQueue = eventbus.NewEventQueue("channel-timeout-test")
