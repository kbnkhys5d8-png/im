...
type Options struct {
	...
	// ConnectionAuthorizer runs before a UID is installed in ConnManager.
	// Returning an error rejects and closes the connection without replacing an
	// existing connection that owns the same UID.
	ConnectionAuthorizer func(uid string, conn gnet.Conn) error
...
func WithConnectionAuthorizer(authorizer func(uid string, conn gnet.Conn) error) Option {
	return func(o *Options) {
		o.ConnectionAuthorizer = authorizer
	}
}
