...
func (s *Server) handleConnack(conn gnet.Conn, req *proto.Connect) {
	...
	if s.opts.ConnectionAuthorizer != nil {
		if err := s.opts.ConnectionAuthorizer(req.Uid, conn); err != nil {
			s.Warn("connection authorization rejected", zap.String("uid", req.Uid), zap.Error(err))
			_ = conn.Close()
			return
		}
	}
...
func (s *Server) OnClose(conn gnet.Conn, err error) (action gnet.Action) {
	...
	if !s.ConnManager.RemoveConnIf(connCtx.uid.Load(), conn) {
		return
	}
	s.routeMapLock.RLock()
